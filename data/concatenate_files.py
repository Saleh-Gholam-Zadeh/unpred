...
class frankaArm():
    ...
    def _preprocessFranka(self,observation, action, time_diff, flag=False):
        ...
        act=np.zeros_like(np.array(act))#saleh added
    ...
    def compute_normalization(self,data):
        ...
        """
        print(data['action']) #saleh_added
        data['action']=np.zeros(data['action'])#saleh_added
    ...
    def _load_data(self,split):
        ...
        print("train_prev_act:",train_prev_act)
        train_prev_act=torch.zeros_like(train_prev_act)#saleh_added

        train_target_act = data_train_out[:, 1:-1, :]; test_target_act = data_test_out[:, 1:-1, :]

        print("train_target_act:",train_target_act)
        train_prev_act=torch.zeros_like(train_target_act)#saleh_added
        ...
        print("self.train_prev_acts:",self.train_prev_acts)
        print("self.test_prev_acts",self.test_prev_acts)

        print("self.train_target_acts",self.train_target_acts)
        print("self.test_target_acts",self.test_target_acts)
        ...
        if self.standardize:
            ...
            print("self.normalization['actions'] :",self.normalization['actions']) #saleh_added
            ...
            print(self.train_acts,self.test_acts,self.train_act_diff,self.test_act_diff) #saleh_added
