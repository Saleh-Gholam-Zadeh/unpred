...
class metaMobileData(Dataset):
    def __init__(self, data_cfg=None):
        ...
        self._dataFolder = os.path.join(os.path.dirname(get_original_cwd()) , 'all_data' ,'workload' ) #saleh added to have only 1 data folder for all projects// #os.path.join(get_original_cwd() , 'data' ,'workload' )
        #self._dataFolder = os.path.join(os.path.dirname(os.getcwd()), 'all_data', 'workload')
        self._trajectoryPath = self._dataFolder + '/allArrays_1to50_sz74_19clustered.pickle'
        ...
        self.batch_size = self.c.context_size  # window length/2  # time series window length : 50
        ...
        self.mb = self.c.meta_batch_size
    ...
    def load_data(self):
        ...
        if (self.standardize or self.standardize_dist):  # True
            ...
            if(self.standardize and not(self.standardize_dist)): #only packet normalized
                self.train_obs = train_obs #we need to define it and then we have access to it's slice
                self.train_obs[:,:,-1:]  = self.normalize(train_obs[:,:,-1:], self.normalization["packets"][0],self.normalization["packets"][1])

                self.train_acts = self.normalize(train_act[:,:,:],   self.normalization["actions"][0],self.normalization["actions"][1])

                self.test_obs = test_obs  #we need to define it and then we have access to it's slice
                self.test_obs[:,:,-1:]   = self.normalize(test_obs[:,:,-1:], self.normalization["packets"][0],self.normalization["packets"][1])
            ...
            if (self.tar_type == 'observations' and self.features_out=='packets'):
                ...
                print("train_obs[:]||test_obs[:]|| ----->   input(X) and target(Y)  packets are normalized")
                print("train_next_obs([:-1])||test_next_obs[:-1]|| -----> input X and target Y Distribution are Not normalized")
        ...
        if self._save_windows is not None:
            ...
            txt_train = 'train_data_window_' +str(self.mb) +str(self.l.features_in)+"->"+str(self.l.features_out)+ '.npz'
            txt_test  = 'test_data_window_'  +str(self.mb) +str(self.l.features_in)+"->"+str(self.l.features_out)+ '.npz'
    ...
    def _get_batch(self, train, percentage_imputation=0.0): # 3000 random point between 0to50
        ...
        print('--- - -- - - - -  - - - - double_check zero removal -- ---- --- -- - -  - -')
        golden_value = self.normalize(np.zeros(25), self.normalization["packets"][0], self.normalization["packets"][1])[-1]
        #golden_value[:,:,-1:] = self.normalize(np.zeros(25), self.normalization["packets"][0], self.normalization["packets"][1])[-1]
        zeros_cnt=0
