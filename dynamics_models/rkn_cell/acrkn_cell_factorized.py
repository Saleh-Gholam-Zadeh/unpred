...
#    @torch.jit.script_method
    def forward(self, prior_mean: torch.Tensor, prior_cov: Iterable[torch.Tensor], action: torch.Tensor,
        ...
        #print('DEVICEEEEEEEEEEEEEEEEEEEEEEEE', self._tm.device, self._log_transition_noise.device)
    ...
    def _build_transition_model(self) -> None:
        ...
        self._lm = nn.Parameter(torch.zeros(self.c.num_basis, self._lsd, dtype=self._dtype))
        #
        self._om = nn.Parameter(torch.zeros(self._lsd, self.c.num_basis, dtype=self._dtype))

        #self._tm = nn.Parameter(torch.zeros(self._lsd, self._lsd, dtype=self._dtype))
    ...
    def get_transition_model(self, post_mean: torch.Tensor) -> Tuple[List[torch.Tensor], torch.Tensor]:
        """
        Compute the locally-linear transition model given the current posterior mean through factorization
        ...
        #self._gm = torch.diag_embed(coefficients)
        om = self._om.unsqueeze(0).expand(post_mean.shape[0],-1,-1)
        lm = self._lm.unsqueeze(0).expand(post_mean.shape[0], -1, -1)

        #more batch matrix multiplications needed
        #with tsensor.explain():



        ######## IF LOCALLY LINEAR
        #self._tm = torch.bmm(om,torch.bmm(self._gm,lm))




        ######## IF FACTORIZED
        self._tm = torch.bmm(om,lm)
    ...
    def _predict(self, post_mean: torch.Tensor, post_covar: List[torch.Tensor], action: torch.Tensor) \
        ...
        tm11 = tm[:,:self._lod, :self._lod]
        tm12 = tm[:,:self._lod, self._lod:]
        tm21 = tm[:,self._lod:, :self._lod]
        tm22 = tm[:,self._lod:, self._lod:]
    ...
    def _predict_linear(self, post_mean: torch.Tensor, post_covar: List[torch.Tensor], action: torch.Tensor) \
            -> Tuple[torch.Tensor, List[torch.Tensor]]:
        """ Performs prediction step
        :param post_mean: last posterior mean
        :param post_covar: last posterior covariance
        ...
        tm11 = tm[:self._lod, :self._lod].unsqueeze(0).expand(post_mean.shape[0], -1, -1)
        tm12 = tm[:self._lod, self._lod:].unsqueeze(0).expand(post_mean.shape[0], -1, -1)
        tm21 = tm[self._lod:, :self._lod].unsqueeze(0).expand(post_mean.shape[0], -1, -1)
        tm22 = tm[self._lod:, self._lod:].unsqueeze(0).expand(post_mean.shape[0], -1, -1)
