...
from dynamics_models.rkn.acrkn.AcRKNLayer_Simple import AcRKNLayer
...
class AcRKN(nn.Module):
    ...
    def __init__(self, target_dim: int, lod: int, lad: int, cell_config: ConfigDict=None, use_cuda_if_available: bool = True):
        ...
        if self.c.learn_initial_state_covar:
            ...
            self._init_state_covar = \
                nn.Parameter(nn.init.constant_(torch.empty(1, self._lsd), init_state_covar))
        else:
            self._init_state_covar = self.c.initial_state_covar * torch.ones(1, self._lsd)

        self._initial_mean = torch.zeros(1, self._lsd).to(self._device)
        self._initial_cov = torch.nn.Parameter(self._init_state_covar).to(self._device)
    ...
    def forward(self, obs_batch: torch.Tensor, act_batch: torch.Tensor, obs_valid_batch: torch.Tensor) -> Tuple[float, float]:
        ...
        post_mean, post_cov, prior_mean, prior_cov  = self._rkn_layer(w, w_var, act_batch, self._initial_mean, self._initial_cov, obs_valid_batch)
        out_mean, out_var = self._dec(prior_mean, prior_cov)
